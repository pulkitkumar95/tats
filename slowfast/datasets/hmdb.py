...
class Hmdb51(torch.utils.data.Dataset):
    """
    Hmdb51 1-1 video loader. Construct the 101 video loader,
    ...
    def __init__(self, cfg, mode, num_retries=10, sure=False):
        """
        Load UCF data (frame paths, labels, etc. )
        Args:
            cfg (CfgNode): configs.
        ...
        csv_name = 'few_shot_main_data_csvs//hmdb_few_shot.csv' if cfg.TASK == 'few_shot' else  cfg.DATA.DATA_CSV_NAME 
    ...
    def _construct_loader(self):
        ...
        self.split_df = self.split_df[self.split_df['feat_exists']].reset_index(
                                                                    drop=True)
        self.split_df['video_path'] = self.split_df['vid_base_name'].apply(lambda x: os.path.join(self.data_root, x))
